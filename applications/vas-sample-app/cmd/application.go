...
func postVAServingRequest(vaEndPoint string, vaPipeline string) error {
	...
	for postReconnectTries := 0; postReconnectTries < 10; postReconnectTries++ {
		if VASPostResp.StatusCode == http.StatusServiceUnavailable {
	...
	for {
		...
		for getReconnectTries := 0; getReconnectTries < 10; getReconnectTries++ {
			if VASGetResp.StatusCode == http.StatusServiceUnavailable {
